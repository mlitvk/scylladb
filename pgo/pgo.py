...
async def train_full(executable: PathLike, output_profile_file: PathLike, dataset_dir: PathLike,
                     workloads: Optional[list[str]] = None) -> None:
    ...
    `workloads` restricts the run to some of them, which is for working on one
    workload rather than for producing a profile: a profile is only as good as the
    coverage of the workloads it was trained on.
    ...
    workloads = workloads if workloads is not None else list(trainers)
    if unknown := [w for w in workloads if w not in trainers]:
        raise ValueError(f"Unknown training workloads {unknown}. Known ones are {list(trainers)}.")
...
async def main() -> None:
    try:
        match sys.argv[1]:
            case "train_full":
                # The optional fifth argument is a comma separated list of workloads to
                # run instead of all of them. For working on one workload only - the
                # profile it produces covers nothing but that workload.
                workloads = sys.argv[5].split(",") if len(sys.argv) > 5 else None
