...
future<mutation> get_switch_streams_mutation(table_id table, db_clock::time_point stream_ts, const cdc_stream_diff& diff, api::timestamp_type ts) {
...
future<> generation_service::generate_tablet_resize_update(utils::chunked_vector<canonical_mutation>& muts, table_id table, const locator::tablet_map& new_tablet_map, api::timestamp_type ts) {
    ...
    new_ts = std::max(new_ts, current_streams.ts + std::chrono::milliseconds(1));

    auto diff = co_await _cdc_metadata.generate_stream_diff(current_streams.streams, new_streams);
    auto mut = co_await get_switch_streams_mutation(table, new_ts, diff, ts);
