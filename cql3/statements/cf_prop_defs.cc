...
const sstring cf_prop_defs::KW_LOG_STRUCTURED_STORAGE = "log_structured_storage";
...
void cf_prop_defs::validate(const data_dictionary::database db, sstring ks_name, const schema::extensions_map& schema_extensions) const {
    ...
    if (has_property(KW_LOG_STRUCTURED_STORAGE)) {
        bool use_log_structured = get_boolean(KW_LOG_STRUCTURED_STORAGE, false);
        if (use_log_structured && !db.get_config().enable_log_structured_storage()) {
            throw exceptions::configuration_exception(format(
                "'{}' property is set but log structured storage is disabled",
                KW_LOG_STRUCTURED_STORAGE));
        }
    }
}
...
void cf_prop_defs::apply_to_builder(schema_builder& builder, schema::extensions_map schema_extensions, const data_dictionary::database& db, sstring ks_name, bool supports_repair) const {
    ...
    if (has_property(KW_LOG_STRUCTURED_STORAGE)) {
        bool use_log_structured = get_boolean(KW_LOG_STRUCTURED_STORAGE, false);
        if (use_log_structured) {
            builder.set_log_structured_storage_enabled(true);
        }
    }
}
