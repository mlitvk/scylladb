...
public:
    ...
    future<owned_write_buffer> allocate(std::string_view buffer_name) {
        auto units = co_await get_units(_available_sem, 1);
        if (_available.empty()) {
            throw std::runtime_error(fmt::format("No available {} after acquiring semaphore unit", buffer_name));
        }
        auto* wb = _available.back();
        _available.pop_back();
...
struct compaction_buffer {
    ...
    static future<compaction_buffer> allocate(segment_manager_impl& sm, logstor_group& cg) {
        auto buf = co_await sm._compaction_buffer_pool.allocate("compaction buffer");
