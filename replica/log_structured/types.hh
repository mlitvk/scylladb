...
struct log_location {
    ...
    auto operator<=>(const log_location& other) const noexcept = default;
};
...
struct log_structured_segment_record {
    index_key key;
...
template <>
struct fmt::formatter<replica::log_structured::log_segment_id> : fmt::formatter<uint64_t> {
    template <typename FormatContext>
    auto format(const replica::log_structured::log_segment_id& id, FormatContext& ctx) const {
        return fmt::formatter<uint64_t>::format(id.value, ctx);
    }
};

template <>
struct fmt::formatter<replica::log_structured::log_location> : fmt::formatter<string_view> {
    template <typename FormatContext>
    auto format(const replica::log_structured::log_location& loc, FormatContext& ctx) const {
        return fmt::format_to(ctx.out(), "{{segment:{}, offset:{}, size:{}}}",
                             loc.segment.value, loc.offset, loc.size);
    }
};

template <>
struct fmt::formatter<replica::log_structured::index_key> : fmt::formatter<string_view> {
    template <typename FormatContext>
    auto format(const replica::log_structured::index_key& key, FormatContext& ctx) const {
