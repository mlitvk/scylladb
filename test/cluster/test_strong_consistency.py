...
async def test_tablet_migration_rollback(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 3} AND tablets = {'initial': 1} AND consistency = 'global'") as ks:
        async with new_test_table(manager, ks, "pk int PRIMARY KEY, c int") as table:
            ...
            src_host_id, src_shard = next((host_id, shard) for host_id, shard in original_replicas if host_id in racks[0])
            dst_host_id = next(host_id for host_id in racks[0] if host_id != src_host_id)
            dst_shard = 0

            logger.info(
                "Triggering rollback during streaming by migrating replica from %s:%s to %s:%s",
            ...
            await manager.api.enable_injection(servers[0].ip_addr, "stream_tablet_move_to_cleanup", one_shot=True)
