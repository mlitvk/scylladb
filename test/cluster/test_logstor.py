...
async def test_space_accounting_metrics(manager: ManagerClient):
    """
    Verify that scylla_logstor_sm_live_index_bytes reflects the live durable data
    size after the initial unique-key load and remains unchanged after many
    fixed-size overwrites that fill the disk multiple times and after a server restart.
    """
    disk_size_mb = 4
    file_size_mb = 1
    key_count = 800
    value_size = 2000
    value_overhead_budget = 500
    overwrite_rounds = 5
    ...
    async def get_live_index_bytes() -> float:
        metrics = await manager.metrics.query(server.ip_addr)
        live_index_bytes = metrics.get("scylla_logstor_sm_live_index_bytes")
        live_record_count = metrics.get("scylla_logstor_sm_live_record_count")
        assert live_index_bytes is not None
        assert live_record_count is not None
        return live_index_bytes, live_record_count
    ...
    async with new_test_keyspace(manager, "WITH tablets={'initial':1}") as ks:
        ...
        baseline_live_index_bytes, baseline_live_record_count = await get_live_index_bytes()
        logger.info(f"baseline live_index_bytes={baseline_live_index_bytes}, baseline_live_record_count={baseline_live_record_count}, avg overhead per record={(baseline_live_index_bytes / baseline_live_record_count) - value_size}")

        assert baseline_live_record_count == key_count, (
            f"expected live_record_count to be {key_count}, got {baseline_live_record_count}"
        )

        assert baseline_live_index_bytes >= key_count * value_size, (
            f"expected live_index_bytes to be at least {key_count * value_size}, "
            f"got {baseline_live_index_bytes}"
        )
        # live_index_bytes tracks full durable record bytes, so allow room for
        # mutation and log record metadata on top of the raw value payload.
        assert baseline_live_index_bytes <= key_count * (value_size + value_overhead_budget), (
            f"expected live_index_bytes to be at most {key_count * (value_size + value_overhead_budget)}, "
            f"got {baseline_live_index_bytes}"
        )

        for round_no in range(overwrite_rounds):
            value = make_value(f"round_{round_no}_")
            await asyncio.gather(*[cql.run_async(insert, [pk, value]) for pk in range(key_count)])

            await manager.api.logstor_flush(server.ip_addr)

            final_live_index_bytes, final_live_record_count = await get_live_index_bytes()
            logger.info(f"final live_index_bytes={final_live_index_bytes}, final_live_record_count={final_live_record_count}")

            assert final_live_record_count == key_count, (
                f"expected live_record_count to remain {key_count} after overwrites, "
                f"got {final_live_record_count}"
            )

            assert final_live_index_bytes == baseline_live_index_bytes, (
                f"expected live_index_bytes to remain {baseline_live_index_bytes} after overwrites, "
                f"got {final_live_index_bytes}"
            )
        ...
        final_live_index_bytes_after_restart, final_live_record_count_after_restart = await get_live_index_bytes()
        logger.info(f"final_live_index_bytes_after_restart={final_live_index_bytes_after_restart}, final_live_record_count_after_restart={final_live_record_count_after_restart}")
        ...
        assert final_live_index_bytes_after_restart == baseline_live_index_bytes, (
            f"expected live_index_bytes to remain {baseline_live_index_bytes} after restart, "
            f"got {final_live_index_bytes_after_restart}"
        )
