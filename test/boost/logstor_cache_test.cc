...
std::vector<dht::decorated_key> insert_same_token_keys(primary_index& index, simple_schema& schema, int64_t token, int start, int count) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        auto [ok, prev] = index.insert(primary_index_key{dk}, make_index_entry(500 + i, i * 8, 8, 1), accounting);
...
SEASTAR_THREAD_TEST_CASE(test_logstor_primary_index_cache_invalidation_and_eviction) {
    ...
    BOOST_REQUIRE(!index.insert(primary_index_key{dk0}, make_index_entry(1, 0, 10, 1), accounting).second);
    BOOST_REQUIRE(!index.insert(primary_index_key{dk1}, make_index_entry(1, 10, 10, 1), accounting).second);
    BOOST_REQUIRE(!index.insert(primary_index_key{dk2}, make_index_entry(1, 20, 10, 1), accounting).second);
    BOOST_REQUIRE(!index.insert(primary_index_key{dk3}, make_index_entry(1, 30, 10, 1), accounting).second);
    ...
    auto [inserted1, old_entry1] = index.insert(primary_index_key{dk1}, make_index_entry(2, 10, 12, 2), accounting);
    BOOST_REQUIRE(inserted1);
    BOOST_REQUIRE(old_entry1);
    BOOST_REQUIRE(!lookup_exists(index, cache.logstor_tracker, dk1, schema.schema()));

    auto [inserted2, old_entry2] = index.insert(primary_index_key{dk2}, make_index_entry(2, 20, 12, 2), accounting);
    BOOST_REQUIRE(inserted2);
    BOOST_REQUIRE(old_entry2);
    BOOST_REQUIRE(!lookup_exists(index, cache.logstor_tracker, dk2, schema.schema()));

    auto [inserted0, old_entry0] = index.insert(primary_index_key{dk0}, make_index_entry(3, 0, 14, 0), accounting);
    BOOST_REQUIRE(!inserted0);
    BOOST_REQUIRE(old_entry0);
    BOOST_REQUIRE(lookup_exists(index, cache.logstor_tracker, dk0, schema.schema()));

    BOOST_REQUIRE(index.erase(primary_index_key{dk1}, make_index_entry(1, 10, 10, 1).location, accounting) == false);
    BOOST_REQUIRE(index.erase(primary_index_key{dk1}, make_index_entry(2, 10, 12, 2).location, accounting));
    ...
    index.erase(dht::partition_range::make_singular(dk2), accounting).get();
...
SEASTAR_THREAD_TEST_CASE(test_logstor_primary_index_cache_survives_index_rebalancing) {
    ...
    for (int i = 0; i < 48; ++i) {
        BOOST_REQUIRE(index.erase(primary_index_key{grown_keys[i]}, make_index_entry(500 + i, i * 8, 8, 1).location, accounting));
    }
...
SEASTAR_THREAD_TEST_CASE(test_logstor_cache_survives_lsa_compaction_before_exchange) {
    ...
    auto [replaced, old_entry] = index.insert(primary_index_key{keys[2]}, make_index_entry(900, 0, 32, 2), accounting);
...
SEASTAR_THREAD_TEST_CASE(test_logstor_cache_upgrades_cached_partition_after_schema_change) {
    ...
    BOOST_REQUIRE(!index.insert(primary_index_key{dk}, make_index_entry(1, 0, 10, 1), accounting).second);
